...
export class PortfolioPortMockAdapter implements PortfolioPort {
    ...
    async create(command: CreatePortfolioCommand): Promise<Portfolio> {
        const newPortfolio = new Portfolio(command.userId, command.name, [], command.userId);

        return newPortfolio
    }
    async addNewShareToPortfolio(command: AddShareToPortfolioCommand): Promise<Portfolio> {
    ...
    async updatePortfolioShareItem(command: PortfolioShareItem): Promise<Portfolio> {
        const portfolio = this.portfolios.find(p => p.id === command.portfolioId);
        ...
        const shareItem = portfolio.shareItems.find(s => s.share.id === command.share.id);
    ...
    async removeShareFromPortfolio(shareId: number, portfolioId: number): Promise<Portfolio> {
        ...
        portfolio.shareItems = portfolio.shareItems.filter(s => s.share.id !== shareId);

        return portfolio
    }
    async retrievePortfolioByIdAndUserId(portfolioId: number, userId: number): Promise<Portfolio> {
